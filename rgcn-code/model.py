...
class  RelGraphConvLayer(nn.Module):
    ...
    layer_position: position of layer in the model
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.conv = dglnn.HeteroGraphConv(
            {
                rel: dglnn.GraphConv(in_feat, out_feat, norm="right", weight=False, bias=False) for rel in rel_names
            }
        )
        empt = {}
        for i, name in enumerate(rel_names):
            # if "house" in name:
            #     empt[name] = dglnn.GraphConv(23, out_feat, norm="right", weight=False, bias=False)
            # else:
            empt[name] = dglnn.GraphConv(23, out_feat, norm="right", weight=False, bias=False)
        if self.layer_position == 1:
            self.conv = dglnn.HeteroGraphConv(empt)
    ...
    def forward(self, g, inputs):
        ...
        def _apply(ntype, h):
            ...
            if self.bias:
                # print("matmul is", self.h_bias.size())
                # print("h size",h.size())
                h = h + self.h_bias
            if self.activation:
                h = self.activation(h)
...
class RelGraphEmbed(nn.Module):
    ...
    def __init__(
        ...
        for ntype in g.ntypes:
            ...
            # added lines
            if ntype == 'house':
                embed = self.g.nodes['house'].data['feat']
            if ntype == 'park':
                embed = self.g.nodes['park'].data['feat']
            if ntype == 'hospital':
                embed = self.g.nodes['hospital'].data['feat']
...
class EntityClassify(nn.Module):
    def __init__(
        ...
        # self.embed_layer = RelGraphEmbed(g, 23) # testing new input shape
        self.embed_layer = RelGraphEmbed(g, 13)
        self.layers = nn.ModuleList()
        # i2h
        self.layers.append(
            RelGraphConvLayer(
                13, # testing new input shape
                self.h_dim,
                self.rel_names,
                self.num_bases,
                activation=F.relu,
        ...
        self.layers.append(
            RelGraphConvLayer(
                ...
                activation=None,
                self_loop=self.use_self_loop,
                layer_position= self.num_hidden_layers+2
            )
        )
        #self.layers.append(nn.Linear(h_dim, self.out_dim))
            ...
            # full graph training
             for i, layer in enumerate(self.layers):
                # if i == len(self.layers)-1:
                    ...
                    #         hg = hg + dgl.mean_nodes(self.g, 'h', ntype=ntype) # part of hyperparameter tuning 
                    #     print(hg)
                    #     return layer(hg)
                # print(h)
                h = layer(self.g, h)
...
# store the prices in the features of the nodes
# have the prices as the target value anyway
# predict 1 value for each node you will regress for.
# do it